...
## 11. Mijoz ma'lumotlarini tahrirlash (Edit Client Profile)

Mavjud mijozning shaxsiy profil ma'lumotlarini (ismi, jinsi, PINFL, tug'ilgan sanasi, vazni, bo'yi va rasmi) yangilash / tahrirlash uchun ishlatiladi. Ushbu API avtorizatsiya talab qiladi.

- **URL:** `/mobile/v1/put/client`
- **Method:** `PUT`
- **Headers:** 
  - `Authorization: Bearer <token>`
  - `Content-Type: application/json`
...
| Parametr | Turi | Majburiymi? | Izoh |
| :--- | :--- | :---: | :--- |
| `id` | `string` (UUID) | Ha | Tahrirlanayotgan mijozning unikal identifikatori |
| `name` | `string` | Ha | Mijozning to'liq ismi sharifi. Masalan: `Ali Valiyev` |
| `gender` | `string` | Ha | Jinsi (`male` yoki `female`) |
| `pinfl` | `number` / `integer` | Ha | 14 xonali JSHSHIR (PINFL). Masalan: `12345678901234` |
| `birth_date` | `string` (Date) | Yo'q | Tug'ilgan sanasi (`YYYY-MM-DD` formatida). Masalan: `2001-10-24` |
| `weight` | `number` / `numeric` | Yo'q | Vazni (kg da). Masalan: `100.0` |
| `height` | `number` / `integer` | Yo'q | Bo'yi (sm da). Masalan: `193` |
| `pic` | `string` (UUID) | Yo'q | Yuklangan rasm identifikatori |

**Request namunasi (cURL):**
```bash
curl --location --request PUT 'https://zal360.uz/endpoint/api/mobile/v1/put/client' --header 'Authorization: Bearer eyJra...' --header 'Content-Type: application/json' --data '{
  "id": "9b9f2fd9-6ffb-4b9d-b5bc-3f4aaaa121f6",
  "name": "Ali Valiyev",
  "gender": "male",
  "pinfl": 12345678901234,
  ...
  "weight": 100.0,
  "height": 193,
  "pic": "3d2c94b3-487e-4de8-8451-d7750177653f"
}'
```
...
#### 1. Muvaffaqiyatli javob (200 OK)
Ma'lumotlar muvaffaqiyatli yangilanganda yangilangan mijozning `id` si (`upd.id`) qaytadi:

```json
{
    "error": null,
    "message": null,
    "timestamp": "2026-08-19T05:09:08.585+00:00",
    "code": null,
    "path": null,
    "data": {
        "response": {},
